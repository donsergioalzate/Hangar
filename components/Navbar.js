    ...
    <nav className="sticky top-0 z-50 bg-white border-b-4 border-black">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          {/* Cart */}
          <Link href="/carrito" className="relative flex items-center gap-1 bg-black text-[#ffc832] border-2 border-black px-3 py-1.5 font-bold text-sm shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
            <ShoppingCart size={16} />
            <span className="hidden sm:inline">Carrito</span>
            {itemCount > 0 && (
              <span className="bg-[#ffc832] text-black text-xs font-black px-1.5 rounded-full border-2 border-black">{itemCount}</span>
            )}
          </Link>
          ...
          {session ? (
            <div className="flex items-center gap-2">
              {session.user.role === 'ADMIN' && (
                <Link href="/dashboard" className="hidden sm:flex items-center bg-black text-[#ffc832] border-2 border-black px-3 py-1.5 font-bold text-sm shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
                  Admin
                </Link>
              )}
